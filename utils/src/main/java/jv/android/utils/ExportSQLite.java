...
public class ExportSQLite {
	...
	public ExportSQLite( Context ctx, SQLiteDatabase db, String fileName )
	{
		_ctx = ctx;
		_db = db;
		
		if (!fileName.contains("/"))
			EXPORT_FILE_NAME = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + fileName;
		else
...
//		Log.d( "DatabaseAssistant", msg );
    ...
    public static boolean exportBackup (String databaseFile, String backupFile) {
    	File dbFile = null;             
    	File file = null;           

    	if (!databaseFile.contains("/")) {
    		dbFile = new File(Environment.getDataDirectory(), databaseFile);           
    	} else {
    		dbFile = new File(databaseFile);           
    	}    	
    	
    	if (!backupFile.contains("/")) {
        	file = new File(Environment.getExternalStorageDirectory(), backupFile);           
    	} else {
