...
public class LatLonUTM {
	...
	public UTM decToUTM(double latitude, double longitude) {
		LatLon2UTM c = new LatLon2UTM();
		return c.latLonToUTM(latitude, longitude);
	}
	...
	private class LatLon2UTM {
		public UTM latLonToUTM(double latitude, double longitude) {
			...
			utm.latZone = latZone;
			utm.easting = (int)_easting;
			utm.northing = (int)_northing;
		...
		public String convertLatLonToUTM(double latitude, double longitude) {
			UTM utm = latLonToUTM(latitude, longitude);

			return utm.longZone + " " + utm.latZone + " " + (utm.easting) + " " + (utm.northing);
		}
